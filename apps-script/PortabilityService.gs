  ...
  try {
    return withDocumentLock_(function() {
      ...
      backup.transactions.forEach(function(row, index) {
        ...
        const base = { request_id: 'migration:' + id + ':' + oldId, date: String(portableValueGs_(row, ['date'], now.slice(0, 10))), time: String(portableValueGs_(row, ['time'], '')), type: String(portableValueGs_(row, ['type'], 'expense')), amount: Math.max(1, Math.round(portableNumberGs_(row, ['amount'], 1))), category: String(portableValueGs_(row, ['category'], 'Lainnya')), merchant: String(portableValueGs_(row, ['merchant', 'title', 'description'], 'Transaksi migrasi')).slice(0, 120), notes: String(portableValueGs_(row, ['notes', 'note'], '')).slice(0, 300), status: String(row.status || 'completed') === 'pending' ? 'pending' : 'completed', created_at: now, updated_at: now, deleted_at: String(portableValueGs_(row, ['deletedAt', 'deleted_at'], '')) };
