...
function apiSaveReportPdf(payload) {
  ...
  try {
    ...
    const allowedSections = ['summary', 'cashflow', 'categories', 'accounts', 'budgets', 'bills', 'goals', 'roadmap', 'forecast', 'emergency', 'debts', 'investments', 'recurring'];
...
function portableSourceGs_(value) {
  ...
  return {
    ...
    recurringTemplates: rows('recurringTemplates', 'recurring_templates', 'Recurring')
  };
}
...
function validatePortableGs_(backup) {
  ...
  backup.recurringTemplates.forEach(function(row, index) {
    ...
    else if (!accountIds[accountId]) errors.push('Transaksi rutin baris ' + (index + 1) + ' merujuk akun yang tidak ada.');
  });
...
function apiApplyMigration(payload) {
  try {
    return withDocumentLock_(function() {
      ...
      appendObjects_(VINN_CONFIG.SHEETS.BILLS, backup.bills.map(function(row, index) { const phases = billInstallmentPhases_(portableValueGs_(row, ['installmentPhases', 'installment_phases', 'installmentPhasesJson', 'installment_phases_json'], [])); const duration = phases.length ? phases.reduce(function(sum, phase) { return sum + phase.durationMonths; }, 0) : Math.max(0, Math.round(portableNumberGs_(row, ['durationMonths', 'duration_months'], 0))); const paidCount = Math.max(0, Math.round(portableNumberGs_(row, ['paidCount', 'paid_count'], 0))); return { id: id_('bill'), name: String(portableValueGs_(row, ['name'], 'Tagihan migrasi ' + (index + 1))).slice(0, 100), amount: Math.max(1, Math.round(portableNumberGs_(row, ['amount'], 1))), category: String(portableValueGs_(row, ['category'], 'Tagihan')), account_id: accountIds[String(portableValueGs_(row, ['accountId', 'account_id'], ''))], frequency: String(portableValueGs_(row, ['frequency'], 'monthly')), due_date: String(portableValueGs_(row, ['dueDate', 'due_date'], now.slice(0, 10))), reminder_days: String(portableValueGs_(row, ['reminderDays', 'reminder_days'], '7,3,1,0')), status: duration && paidCount >= duration ? 'completed' : 'active', last_paid_period: String(portableValueGs_(row, ['lastPaidPeriod', 'last_paid_period'], '')), created_at: now, updated_at: now, liability_account_id: accountIds[String(portableValueGs_(row, ['liabilityAccountId', 'liability_account_id'], ''))] || '', duration_months: duration || '', paid_count: paidCount, current_period_paid: Math.max(0, Math.round(portableNumberGs_(row, ['currentPeriodPaid', 'current_period_paid'], 0))), total_paid: Math.max(0, Math.round(portableNumberGs_(row, ['totalPaid', 'total_paid'], 0))), installment_phases_json: JSON.stringify(phases) }; }));
      ...
      if (backup.settings && typeof backup.settings === 'object') {
        ...
        if (backup.settings.notificationEmailEnabled !== undefined) upsertSetting_('notification_email_enabled', backup.settings.notificationEmailEnabled === true);
        if (backup.settings.notificationEmailAddress !== undefined) upsertSetting_('notification_email_address', String(backup.settings.notificationEmailAddress || '').trim().slice(0, 160));
        if (backup.settings.notificationWeeklyDigest !== undefined) upsertSetting_('notification_weekly_digest', backup.settings.notificationWeeklyDigest !== false);
