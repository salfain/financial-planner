...
function dashboardCacheKey_(period) {
  ...
  // Scope anggota ikut menjadi kunci agar cache satu anggota tidak terbaca anggota lain.
  const scope = currentScopeMemberId_();
  return 'dashboard:' + dashboardCacheVersion_() + ':' + month + (scope ? ':' + scope : '');
}
