...
import { IMPORT_BATCH_SIZE, previewTransactionCsv, previewTransactionRecords, transactionCsvTemplate, type TransactionImportPreview } from "../lib/transaction-import";
...
function TransactionImportModal({ accounts, categories, categoryRules, existingTransactions, saving, onClose, onSubmit }: {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="modal import-modal" role="dialog" aria-modal="true" aria-labelledby="import-title">
      ...
      {preview && <>
        <div className="import-summary"><span><small>Siap diimpor</small><strong>{preview.validCount}</strong>{preview.validCount > IMPORT_BATCH_SIZE && <em>dikirim {Math.ceil(preview.validCount / IMPORT_BATCH_SIZE)} batch</em>}</span><span><small>Sudah ada</small><strong>{preview.duplicateCount}</strong></span><span className={preview.errorCount ? "negative-text" : "positive-text"}><small>Perlu diperbaiki</small><strong>{preview.errorCount}</strong></span><span><small>Pengeluaran baru</small><strong>{formatIDR(preview.expense)}</strong></span></div>
