...
export function FinanceApp() {
  ...
  const applySnapshot = (snapshot: Awaited<ReturnType<typeof loadFinanceSnapshot>>) => {
    ...
    setCoupleMode(snapshot.coupleMode);
  };
  ...
  return (
    <div className="app-shell">
      ...
      {(accountOpen || editingAccount) && <AccountModal account={editingAccount ?? undefined} coupleMode={coupleMode} saving={saving} onClose={() => { setAccountOpen(false); setEditingAccount(null); }} onSubmit={async (payload) => { const ok = await runMutation(() => editingAccount ? updateFinanceAccount(editingAccount.id, payload) : createFinanceAccount(payload), editingAccount ? "Akun berhasil diperbarui." : "Akun baru berhasil ditambahkan."); if (ok) { setAccountOpen(false); setEditingAccount(null); } }} />}
  ...
  return <div className="content-stack">
    ...
    <div className="account-grid">
      {accounts.map((account) => <article className={`account-card ${account.liability ? "liability" : ""}`} key={account.id}>
        ...
        <div className="account-card-footer"><span><i style={{ background: account.color }} /> {account.scope === "private" ? "Pribadi" : account.liability ? "Kewajiban" : "Aktif"}</span><button onClick={() => onInspect(account)}>Lihat transaksi <ArrowRight size={14} /></button></div>
      </article>)}
...
function AccountModal({ account, coupleMode, saving, onClose, onSubmit }: { account?: Account; coupleMode: boolean; saving: boolean; onClose: () => void; onSubmit: (payload: Record<string, unknown>) => Promise<void> }) {
