      ...
      d1.prepare(
        `INSERT INTO bills
           (id, workspace_id, name, amount, due_date, category, account_id, frequency, reminder_days, paid, paid_at,
            last_paid_period, liability_account_id, duration_months, paid_count, current_period_paid, total_paid, installment_phases_json, completed, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .bind(
        bill.id,
